...
Every concept has YAML frontmatter with `type`, `title`, `description`, lifecycle metadata, and evidence where applicable. Persisted review state is only `generated` or `reviewed`. The CLI derives `evidence-changed` from a successful tracked-evidence mismatch, `stale` from outdated semantic review metadata or evidence inspection failures, and `attention` from explicit issues.

Acceptance scope is decided by one question — is the page **required** by this spec? — where required means named in `wiki_impact` or touched by the implementation diff. `evidence-changed` is not an acceptance input: it fires whenever any tracked source moves, including the shared hub files that many pages cite, so acceptance advances the baseline of every such page in one mechanical step instead of examining them one by one. Core pages must be anchored on a directory source (`WIKI_CORE_PAGE_UNANCHORED`) so that the pages making aggregate claims about an area become affected when that area actually grows.
