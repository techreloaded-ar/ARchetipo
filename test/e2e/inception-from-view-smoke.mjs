...
async function scenarioInceptionProducesThePRD(runDir) {
  ...
  try {
    ...
    const started = await apiJSON(`${w.view.url}/api/workspace/execution`, postJSON({ action: "inception" }), 201);
    ...
    w.control.push(emit({ type: "assistant", message: { content: [{ type: "text", text: "Ho scritto il PRD." }] } }));
...
async function startViewServer(cwd, env) {
  ...
  // The readiness probe is the workspace actions and not the board: a workspace
  // that has never run an inception has no backlog yet, and `GET /api/board`
  // answers 404 for exactly the workspace this smoke is about.
