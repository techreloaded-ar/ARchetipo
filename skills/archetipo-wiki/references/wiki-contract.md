...
A successfully recomputed hash/revision mismatch produces the non-blocking warning `WIKI_EVIDENCE_CHANGED`. Inspection failures are never labeled as changes: unsafe traversal produces `WIKI_UNSAFE_SOURCE_PATH`, unreadable/unsupported/index/submodule evidence produces `WIKI_EVIDENCE_UNREADABLE`, and a failed legacy Git comparison produces `WIKI_EVIDENCE_RECOMPUTE_FAILED`; these findings have error severity and make `data.ok: false`. `wiki status` still derives `stale` conservatively when recomputation fails. Only `WIKI_EVIDENCE_CHANGED` is eligible for affected-only non-blocking treatment during acceptance.

`wiki affected` applies the same project-relative normalization to tracked sources and changed files, then compares path components. Revision-based discovery consumes NUL-delimited Git names with rename/copy detection disabled, preserving spaces, tabs, newlines, and Unicode. A rename is intentionally observed as deletion plus addition so both old and new paths participate; a copied destination appears as an addition while the unchanged source remains unchanged. A tracked source of `.` or `./` matches every valid project-relative changed file; `src` matches `src/a.go` but never `src2/a.go`. Context sources, valid external HTTP(S) URLs, absolute paths, and out-of-project paths never enter affected discovery.
...
- `archetipo wiki reconfirm <page-id...>` → `kind: wiki_reconfirm_result`, `data.reconfirmed`, `data.root`, `data.pages`; after explicit human verification, refreshes only evidence hash/revision and `reviewed_at` for unchanged, persisted-reviewed, issue-free pages with present evidence and a current semantic content hash. Selection is mandatory and batch preflight is atomic. A current hash is a no-op; a selected legacy no-hash review upgrades. It rebuilds the index and adds one Review log entry only when metadata changed. Never invoke it automatically from spec approval or use it as routine maintenance for references.
...
- `E_CONFLICT`: approval or reconfirmation blocked by validation errors, lifecycle/content eligibility, unresolved issues, missing evidence, unsafe traversal, an unmerged index, an unsupported entry, or a non-clean submodule;
