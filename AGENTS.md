...
- `npm run test:view-execution-smoke`: builds the CLI, initializes a sandbox with two specs (one left `TODO`, one moved to `PLANNED` by a real `spec plan`), starts `archetipo view` on a random port with a sentinel `ARCIPELAGO_TOKEN`, and verifies through the HTTP API that the provider list exposes every registered provider with its configurable fields and no credential — `arcipelago` with its five fields, and the local `codex` with the `spec.plan` and `run.dialog` capabilities, exactly `command`, `model`, `sandbox` and `timeout_seconds`, and the local `claude` with the same capability and exactly `command`, `model`, `print_args` and `timeout_seconds`, none of them with a field whose name asks for a credential — that each provider reports a boolean `available` and that an unavailable one states a non-empty `unavailable_reason` (the smoke observes availability instead of requiring it, so it passes with or without Codex and Claude Code installed, whether or not they are logged in, and never touches `PATH`), that a valid `PUT /api/execution/provider/default` reaches `.archetipo/config.yaml` and is reported afterwards, that an invalid one answers `400` naming the offending `field` while leaving both file and previous default untouched, and that the spec detail exposes exactly the actions the process Template admits — an empty list once the spec is moved to `DONE`.
...
- `npm run test:view-local-run-smoke`: builds the CLI, initializes a `codex` sandbox with one `TODO` spec, starts `archetipo view` on a random port with a sentinel session-material environment variable, and saves as workspace default the real `codex` provider pointed at a **fake Codex binary** (`test/e2e/support/fake-codex.mjs`) that answers `--version` and speaks the real `codex app-server --listen stdio://` JSON-RPC protocol on stdio. It verifies that `codex` declares `run.dialog` beside `spec.plan`; that the session is opened with `approvalPolicy: never` and the configured sandbox, and that the prompt names the spec; that the notifications become an ordered history (`text`, `tool_start`, `tool_end`, `text`) whose cursor returns exactly what is new; that a message posted to `POST /api/execution/{id}/run/messages` reaches the process as a `turn/steer` naming the turn in progress, is absent from the `202` body, and appears exactly once only after the process re-emits it as a user message; that `POST .../run/cancel` reports the run still `ACTIVE` while the process resists and only the end of the turn changes the state; that message and cancel on a run that is over answer `409 run_not_active` and leave the projection byte-identical; and that the sentinel never appears in any viewer response nor in `.archetipo/config.yaml`. Only the Codex binary is fake — the viewer, the connector, the provider, its protocol client, the local session and the run follower are the real ones — the smoke needs no credentials and no traffic outside `127.0.0.1`, the fake never progresses on its own (every notification is commanded through a local control server), and there is no arbitrary sleep. It is skipped on Windows, where the fake's POSIX shebang does not apply.
