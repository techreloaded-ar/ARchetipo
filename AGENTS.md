...
- `wiki-bootstrap-codebase-only`: fixture `fixtures/wiki-codebase`, prompt `/archetipo-wiki`; verifica una mappa DDD codebase-first completa senza documenti prodotto e senza approvazione automatica.
- `wiki-bootstrap-prd-conflict`: fixture `fixtures/wiki-prd-conflict`, prompt `/archetipo-wiki`; verifica archiviazione della sorgente configurata, autorità del codice sullo stato corrente e conflitto registrato come issue.
...
- `npm run test:wiki-smoke`: compila la CLI, ispeziona una codebase sandbox, inizializza la Wiki, crea pagine `generated` ordinarie e `type: decision`, quindi verifica validate, ricerca ADR per tipo, catalog, approvazione selettiva a `reviewed` e rigenerazione di `index.md`.
