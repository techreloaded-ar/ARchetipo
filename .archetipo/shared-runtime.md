...
With the `file` connector, the delivery backlog is also first-class Wiki knowledge under `paths.wiki/backlog/`: `overview.md` is the ordered backlog, `specs/{US-CODE}.md` contains each specification, and `plans/{US-CODE}.md` contains its plan and task contracts. Navigate and search these pages like any other Wiki concept, but mutate their managed frontmatter and workflow state only through `archetipo spec ...` and `archetipo task ...`. Wiki lifecycle `status` (`generated`/`reviewed`) and delivery `workflow_status` (`TODO` through `DONE`) are separate contracts.
