...
// conversationRetainedEvents bounds the history a conversation keeps in
// memory, exactly like claude.conversationRetainedEvents and for the same
// reason: a conversation stays open for as long as somebody keeps it open, so
// an unlimited history would grow with the time the workspace stayed open
// rather than with any unit of work.
...
//
// release exists because two paths can end the same conversation — the
// operator closing it and the process leaving on its own — and both have to
// release the process. Waiting for a process twice is not a thing that can be
// done, so the once is what makes the second path observe the outcome of the
// first instead of asking the operating system a question it has already
// answered.
...
// OpenConversation starts a Codex app server on the requested directory,
// makes it followable under the conversation id and returns without waiting
// for anything.
//
...
// action to satisfy and no receipt to accept. That is what keeps a
// conversation from ever appearing as an execution of the process.
//
// No skill is checked and none is invoked: a free conversation asks the agent
// to read and to answer, and requiring an installed skill for that would
// refuse a conversation a workspace can perfectly well hold.
func (p *Provider) OpenConversation(ctx context.Context, req execution.ConversationRequest) error {
	...
	if id == "" {
		return fmt.Errorf("a codex conversation cannot be opened without an id")
	}
	...
	if _, exists := p.conversations[id]; exists {
		...
		return fmt.Errorf("the codex conversation %q is already open", id)
	}
	...
	}, cfg, dir, buildConversationPrompt(req.ProcessActions, req.Context), true, true, conversationRetainedEvents)
	if err != nil {
...
// the localrun registry, and that is on purpose: the history of a
// conversation that has ended stays readable, and a message sent to it is
// refused with the reason that is true — this run is no longer active —
// instead of the reason that would merely be convenient. It is the rule
// already written on localrun.Registry.
...
// ended.
//
// The state it writes is observed and never deduced, which is the invariant
// of the whole localrun package: it waits for the output of the process to
// end or for the context to be over, releases the process, and only then
// closes the session. A conversation still registered at that moment ended
// without anybody closing it — the process left, or the timeout fired — and
// that is a crash with a reason. One already gone from the map was closed by
// the operator, who has already recorded the ordinary end.
...
// conversationEndReason names why a conversation ended on its own. It is
// never empty: a run reported as crashed without a reason is a run nobody can
// act on.
func (p *Provider) conversationEndReason(live *liveConversation, runErr error) string {
	switch {
	case runErr != nil:
		return fmt.Sprintf("the codex conversation was stopped: %v", runErr)
	case live.waitErr != nil:
		return fmt.Sprintf("the codex conversation process could not be run to completion: %v", live.waitErr)
	default:
		return fmt.Sprintf("the codex conversation process exited %d%s", live.exitCode, localrun.DiagnosticSuffix(live.stderr))
	}
}

// release ends the process of a conversation exactly once and keeps what it
// reported, so the path that did not win the race reads the outcome instead
// of waiting for a process that has already been waited for.
...
// whether it was still there, which is how the two paths that can end one
// tell apart who got there first.
