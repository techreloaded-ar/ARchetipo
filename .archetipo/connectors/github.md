...
This file implements the ARchetipo connector contracts for GitHub Projects. Load it only when `.archetipo/config.yaml` has `connector: github`.
...
- If the calling skill creates backlogs (spec, backlog), create a new project:
  ...
  ```
- Otherwise, stop and tell the user to run `archetipo-spec` first to create the backlog.
...
> **Free-text story creation** is not supported with the GitHub connector. If the argument is not a US-XXX code, inform the user to create the issue on GitHub first, or run `archetipo-spec` to add it to the backlog.
...
```bash
gh issue list --label "archetipo-backlog" --state all --json number,title,labels,body --limit 200
```
...
- `archetipo-backlog` — description: "Story generated by ARchetipo backlog", color: `1D76DB`
...
Use the `archetipo-backlog` label plus the epic label.

```bash
gh issue create --title "US-001: [Story Title]" \
  --label "archetipo-backlog" --label "EP-001: [Epic Title]" \
  --body "$(cat <<'EOF'
## Story
...
gh api graphql -f query='query {
  repository(owner: "$OWNER", name: "$REPO_NAME") {
    issues(labels: ["archetipo-backlog"], last: N, orderBy: {field: CREATED_AT, direction: DESC}) {
...
**Generato da:** ARchetipo Planning Team | **Data:** {DATE}
