...
// runs on a machine that has no Codex on it.
...
func reviewedSession(fake *fakeCodex, specCode string, criteria, blockers int) {
	go func() {
		<-fake.turnStarted
		fake.emit("item/agentMessage/delta", `{"delta":"preparo il dossier"}`)
		fake.emit("item/completed", fmt.Sprintf(`{"item":{"type":"agentMessage","text":%q}}`, reviewReceiptLine(specCode, criteria, blockers)))
		fake.completeTurn()
	}()
}
...
func TestBuildReviewPromptForbidsEveryTransitionByName(t *testing.T) {
	prompt := buildReviewPrompt(reviewRequest("codex"))
...
func TestExecuteReviewRefusesAMissingSkillBeforeSpawning(t *testing.T) {
	...
	_, err := newSessionProvider(dir, &fakeRunner{outcomes: []runOutcome{probeOK}}, fake, nil).Execute(context.Background(), reviewRequest(command))
	if err == nil {
		t.Fatal("expected an error")
	}
	for _, want := range []string{"review skill is not installed", reviewSkillRelPath, "archetipo init --tool codex"} {
...
func TestExecuteReviewReturnsAPayloadBuiltFromTheReceipt(t *testing.T) {
	...
	if got := promptOfTurn(t, fake); got != buildReviewPrompt(req) {
		t.Fatalf("the process received %q; want exactly the review prompt", got)
	}
}
...
func TestExecuteReviewFailureModes(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			wantErr: []string{"exited 1", "without preparing the review of " + testSpec, "codex: not logged in"},
		},
		{
			...
			drive: func(fake *fakeCodex) {
				go func() {
					<-fake.turnStarted
					fake.emit("item/completed", `{"item":{"type":"agentMessage","text":"dossier ready, looks good to me"}}`)
					fake.completeTurn()
				}()
			},
		...
		},
		{
			...
			drive: func(fake *fakeCodex) {
				go func() {
					<-fake.turnStarted
					line := `{"spec_code":"` + testSpec + `","status":"DONE","criteria":5,"blockers":0}`
		...
		},
		{
			...
			drive:   func(fake *fakeCodex) { reviewedSession(fake, "US-999", 4, 0) },
			wantErr: []string{"does not declare a prepared review dossier for " + testSpec},
		},
		{
			name:    "receipt declaring no examined criterion",
			drive:   func(fake *fakeCodex) { reviewedSession(fake, testSpec, 0, 0) },
			wantErr: []string{"does not declare a prepared review dossier for " + testSpec},
		},
		{
