...
1. Run `archetipo wiki --project-root {data.workdir} affected` against the exact spec diff. Treat its page IDs as **discovery candidates**, not as a mass reset or edit list. Keep the plan's `wiki_impact.update` and `wiki_impact.create` requirements distinct from affected-only co-citations. Complete the dedicated implementation task for every required page before code review.
2. Inspect each candidate against the actual semantic change. Reset a reviewed page with `wiki reset` and edit it only when its knowledge is materially obsolete; create every planned page at its canonical ID path. Leave changed or created pages `status: generated` with no `review` metadata. If a tracked source changed but a non-reference page remains accurate, leave its content, persisted status, and review metadata untouched, classify it in the completion summary as an affected-only reconfirmation candidate, and state briefly why the exact diff does not invalidate its claims. If a reference page's tracked original changed, refresh it instead of treating it as a reconfirmation candidate. Context sources should not appear in `wiki affected`; never compensate by discovering or resetting them manually.
...
5. Run `wiki catalog`, then `wiki validate --profile bootstrap`. Repair all errors, strong findings, and coverage warnings caused by the spec. Do not reset an accurate affected-only page merely to erase `WIKI_EVIDENCE_CHANGED`, and never invoke `wiki reconfirm`; acceptance review independently verifies and explicitly reconfirms eligible candidates together with the acceptance verdict.
6. **Prove the state, do not assume the action.** Build the **required** page ID list with the same definition acceptance review uses: an ID listed in the plan's `wiki_impact.update` or `wiki_impact.create`, **or** a Wiki Markdown file created or modified by the implementation diff. Then run `archetipo wiki --project-root {data.workdir} status --require-generated <required-id>...`. It returns `ok: true` only when every listed page reports `state: generated`; otherwise it fails with `E_CONFLICT` and names the offending IDs. A required page reported as `reviewed`, `stale`, or `evidence-changed` was edited without `wiki reset`: run `archetipo wiki --project-root {data.workdir} reset <page-id>...` — reset is idempotent and repairs the state after the edit exactly as it would have before it — and re-run the check. A required page reported as `attention` is an unresolved page issue, not a reset case: fix the issue. `wiki validate --profile bootstrap` returning `ok: true` does not cover any of this, because `WIKI_REVIEW_OUTDATED` and `WIKI_EVIDENCE_CHANGED` are warnings and never make `ok` false. This check covers required pages only: affected-only pages keep their persisted status per point 2 and 5 and are reconciled by acceptance review. Do not enter code review while the check fails.
