...
| Required | absent, stale, attention, invalid, missing evidence | blocker |
| Affected-only | reviewed/fresh | already reviewed |
| Affected-only | stale with only `WIKI_EVIDENCE_CHANGED` | persistent warning; non-blocking |
...
Do not create backlog specs or Wiki log entries merely because a page is affected-only. Persistent stale state is the follow-up signal.
...
5. **The Wiki acceptance dossier.** For every page in the Wiki review set, show one compact entry containing:
   ...
   - verdict readiness: `ready`, `already reviewed`, `warning`, or `blocked`, with the concrete remedy.

   Affected-only evidence-stale warnings stay visibly stale after acceptance; do not present them as accepted or silently repaired. Affected-only generated pages are not approval candidates.
...
State that **Approve** accepts both the delivered increment and the **required** Wiki pages marked `ready` in the dossier. Pass only required-ready IDs to approval. Affected-only warnings remain follow-up signals and do not remove Approve; blockers from the matrix do. If blockers exist, do not offer Approve: present them before asking and offer only **Request changes** or **Postpone**. If the user's initial request already contains an explicit verdict, treat it as the answer only after building and presenting the dossier; execute it immediately when eligible, but never bypass the dossier or blockers.
...
- Immediately run `wiki --project-root {data.workdir} status` again and require every approved ID to report `state: reviewed`, with review metadata present in its frontmatter. Also require affected-only warnings to retain their prior lifecycle/review metadata and expected derived state. Review never edits sources, issues, coverage, or page content merely to make approval pass: a failed approval is a blocker and must become request-changes or postpone. Never run `wiki reconfirm` as part of this flow.
- `archetipo wiki approve` is the only writer for review metadata, `index.md`, and `log.md`. Never synthesize, replace, or reformat those artifacts by hand. The generated log must retain the `# Wiki Update Log` heading and contain the current `* **Review**:` entry below an ISO date heading such as `## 2026-07-16`; run `archetipo wiki --project-root {data.workdir} validate` after approval and stop if it reports `WIKI_LOG_FORMAT`.
- When a worktree is active, approval changes the reviewed page files plus `<paths.wiki>/index.md` and `<paths.wiki>/log.md` inside `data.workdir`. Build the exact path list from the approved IDs, add both generated catalog files even when `log.md` is new and untracked, and run `git add -- <approved-page-paths> <paths.wiki>/index.md <paths.wiki>/log.md`. Before committing, use `git diff --cached --name-only -- <paths.wiki>` and require every approved page, `index.md`, and `log.md` to be present; also require the staged `log.md` to contain the CLI-produced current `**Review**` entry. Stage no unrelated files. Create a commit on the spec branch with subject `docs({US-CODE}): approve Wiki updates`, then require `git status --short -- <paths.wiki>` to be empty. Stop instead of integrating if any Wiki change, including an untracked log, remains outside the commit.
