...
`wiki affected` applies the same project-relative normalization to tracked sources and changed files, then compares path components. A tracked source of `.` or `./` matches every valid project-relative changed file; `src` matches `src/a.go` but never `src2/a.go`. Context sources, external URLs, absolute paths, and out-of-project paths never enter affected discovery.
