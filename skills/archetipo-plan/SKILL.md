...
This single command saves the plan AND transitions the spec to `{config.workflow.statuses.planned}` atomically (and clears the rework marker) — no separate `status set` step is needed. The CLI persists according to the active connector (file: writes the indexed Wiki page `{paths.wiki}/backlog/plans/{US-CODE}.md`; github: appends to the parent issue body and creates one sub-issue per task). For the file connector, follow the template in `./references/plan-template.md` to compose `plan_body` (technical solution + baseline + acceptance evidence map + test strategy — no task summary table).
