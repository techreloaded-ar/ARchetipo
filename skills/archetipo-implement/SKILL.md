...
5. Run `wiki catalog`, then `wiki validate --profile bootstrap`. Repair all errors, strong findings, and coverage warnings caused by the spec. Do not reset an accurate affected-only page merely to erase `WIKI_EVIDENCE_CHANGED`, and never invoke `wiki reconfirm`; acceptance review independently verifies and explicitly reconfirms eligible candidates with the human verdict.
