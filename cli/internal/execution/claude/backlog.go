...
func (p *Provider) executeBacklog(ctx context.Context, req execution.Request, cfg settings, dir string) (execution.Result, error) {
	...
	live, err := p.openSession(runCtx, req, cfg, dir, buildBacklogPrompt(req), true, false, 0)
	if err != nil {
		return execution.Result{}, err
	}
	...
	if err != nil {
		...
		return execution.Result{}, fmt.Errorf("the claude command %q ended without having produced a backlog%s: %w", cfg.Command, localrun.DiagnosticSuffix(stderr), err)
	}
