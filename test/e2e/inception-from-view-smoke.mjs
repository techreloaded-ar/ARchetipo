...
// End-to-end smoke for "start the inception from the UI and get the PRD"
// (US-040).
//
...
// client, the local session, the server-side run follower, the workspace
// routes (`GET /api/workspace/actions`, `POST /api/workspace/execution`), the
// dialogue routes (`GET /api/execution/{id}/run`, `POST .../run/messages`,
// `POST .../run/cancel`) and `GET /api/prd`. Only the agent binary is replaced,
// by `support/fake-claude.mjs`, which speaks the same protocol on stdio, so the
// inception needs no credential and no network.
//
...
//
// Three sandboxes, because the five criteria are three different workspaces:
// one that has no PRD and gets one (AC-1, AC-2, AC-3), one whose run is
// cancelled halfway (AC-4), and one that already has a PRD (AC-5).
...
const defaultWorkspaceRoot = path.join(repoRoot, "test", "workspaces", "inception-from-view-smoke");
...
const AUTH_SENTINEL = "claude-inception-session-material-DO-NOT-EXPOSE";

const QUESTION = "Chi è la persona che userà questo prodotto?";
const ANSWER = "smoke-inception-answer-sentinel";
const PRD_PATH = path.join("docs", "PRD.md");
const PRD_BODY = [
...
const PARTIAL_PRD_BODY = "# PRD — bozza interrotta\n\nSolo metà documento.\n";
const RECEIPT = JSON.stringify({ artifact: "prd", status: "WRITTEN", path: "docs/PRD.md" });
const PRE_EXISTING_PRD_BODY = "# PRD già scritto a mano\n\nQuesto documento esisteva prima.\n";
...
async function main() {
  ...
  let failure = null;
  try {
    await scenarioInceptionProducesThePRD(runDir);
    await scenarioCancelledRunLeavesNoPRD(runDir);
    await scenarioExistingPRDIsNeverOverwritten(runDir);
  ...
  console.log(`\nPASS: inception-from-view smoke test completed (${checks.length} statements proved).`);
  console.log(`Sandboxes: ${runDir}`);
}

// AC-1, AC-2 and AC-3 — a workspace with no PRD offers the inception, the
// agent's question and the person's answer are one conversation, and the PRD is
// readable from the same viewer process the moment the run succeeds.
async function scenarioInceptionProducesThePRD(runDir) {
  console.log("\n=== AC-1/AC-2/AC-3: the inception produces the PRD ===");
  const w = await openWorkspace(runDir, "sandbox-prd");
  try {
    // AC-1 — a workspace with no PRD offers the inception at workspace level.
    const actions = await apiJSON(`${w.view.url}/api/workspace/actions`);
    if (actions.has_prd !== false) {
      throw new Error(`AC-1: a fresh workspace must report has_prd:false; got ${JSON.stringify(actions)}`);
    }
    const inception = (actions.actions || []).find((action) => action.id === "inception");
    if (!inception || inception.runnable !== true || inception.unavailable_reason) {
      throw new Error(`AC-1: the inception must be offered as runnable; got ${JSON.stringify(inception)}`);
    }
    ...
    ok("AC-1", `a workspace without a PRD offers ${JSON.stringify(inception.label)} as runnable, with no execution behind it`);

    // The run starts from the state the UI produces: a press on that action.
    const started = await pressAction(w, { action: "inception" });
    ...
    if (!prompt.includes("/archetipo-inception")) {
      throw new Error(`AC-1: the prompt does not invoke the inception skill: ${JSON.stringify(prompt)}`);
    }
    ok("AC-1", `pressing the action started ${executionID} as RUNNING with an empty spec_code and gave the process a prompt naming /archetipo-inception`);
    ...
    ok("AC-2", "the answer reached the process as a user frame, was absent from the 202 body, and appears exactly once in the same conversation once re-emitted");

    // AC-3 — the agent persists the PRD and closes on its receipt. The document
    // is written by a command of this smoke, never by the fake, because what is
    // being proved is what the viewer does with a PRD that exists.
    await writePRD(w.sandboxDir, PRD_BODY);
    ...
    if (succeeded.status !== "SUCCEEDED") {
      throw new Error(`AC-3: the inception must succeed; got ${JSON.stringify(succeeded)}`);
    }
    const onDisk = await fs.readFile(path.join(w.sandboxDir, PRD_PATH), "utf8");
    if (onDisk !== PRD_BODY) {
      throw new Error(`AC-3: the PRD on disk is not the document that was written:\n${onDisk}`);
    }
    // The same viewer process, never restarted, is what serves it back.
    const served = await apiJSON(`${w.view.url}/api/prd`);
    if (served.body !== PRD_BODY) {
      throw new Error(`AC-3: GET /api/prd must serve the persisted document; got ${JSON.stringify(served)}`);
    }
    const afterwards = await apiJSON(`${w.view.url}/api/workspace/actions`);
    if (afterwards.has_prd !== true) {
      throw new Error(`AC-3: the workspace must report the PRD it now has; got ${JSON.stringify(afterwards)}`);
    }
    ok("AC-3", `the record closed SUCCEEDED, ${PRD_PATH} holds the document on disk and GET /api/prd serves it back from the same viewer process`);
  } finally {
    await w.close();
  }
}

// AC-4 — a run that is stopped leaves the workspace without a PRD and says why.
async function scenarioCancelledRunLeavesNoPRD(runDir) {
  console.log("\n=== AC-4: a cancelled run leaves no PRD ===");
  const w = await openWorkspace(runDir, "sandbox-cancelled");
  try {
    const started = await pressAction(w, { action: "inception" });
    const executionID = started.id;
    ...
    // The half-written document this run produced before it was stopped.
    await writePRD(w.sandboxDir, PARTIAL_PRD_BODY);
    ...
    if (failed.status !== "FAILED") {
      throw new Error(`AC-4: a stopped inception must fail; got ${JSON.stringify(failed)}`);
    }
    ...
    const stillThere = await fs
      .stat(path.join(w.sandboxDir, PRD_PATH))
      .then(() => true)
      .catch(() => false);
    if (stillThere) {
      throw new Error(`AC-4: the partial PRD must not survive the failed run`);
    }
    const served = await apiJSON(`${w.view.url}/api/prd`);
    if (served.body !== "") {
      throw new Error(`AC-4: the workspace must be left without a PRD; GET /api/prd returned ${JSON.stringify(served)}`);
    }
    const actions = await apiJSON(`${w.view.url}/api/workspace/actions`);
    if (actions.has_prd !== false) {
      throw new Error(`AC-4: the workspace must report no PRD after the failure; got ${JSON.stringify(actions)}`);
    }
    ok("AC-4", `the stopped run closed FAILED stating ${JSON.stringify(truncate(failed.error.message))}, ${PRD_PATH} is gone and GET /api/prd is empty`);
  } finally {
    await w.close();
  }
}

// AC-5 — a workspace that already has a PRD is never offered a first inception,
// and nothing of its document is touched by pressing anyway.
async function scenarioExistingPRDIsNeverOverwritten(runDir) {
  console.log("\n=== AC-5: an existing PRD is never overwritten ===");
  const w = await openWorkspace(runDir, "sandbox-existing", async (sandboxDir) => {
    await writePRD(sandboxDir, PRE_EXISTING_PRD_BODY);
  });
  try {
    const before = await fs.readFile(path.join(w.sandboxDir, PRD_PATH));

    const actions = await apiJSON(`${w.view.url}/api/workspace/actions`);
    if (actions.has_prd !== true) {
      throw new Error(`AC-5: the workspace must report the PRD it has; got ${JSON.stringify(actions)}`);
    }
    const inception = (actions.actions || []).find((action) => action.id === "inception");
    if (!inception || inception.runnable !== false) {
      throw new Error(`AC-5: a workspace with a PRD must not offer a first inception; got ${JSON.stringify(inception)}`);
    }
    if (!String(inception.unavailable_reason || "").trim()) {
      throw new Error(`AC-5: an action that is not runnable must say why; got ${JSON.stringify(inception)}`);
    }

    const refused = await expectStatus(`${w.view.url}/api/workspace/execution`, 409, postJSON({ action: "inception" }));
    if (!String(refused.error || "").trim()) {
      throw new Error(`AC-5: the refusal must state a reason; got ${JSON.stringify(refused)}`);
    }

    const after = await fs.readFile(path.join(w.sandboxDir, PRD_PATH));
    if (!before.equals(after)) {
      throw new Error("AC-5: the pre-existing PRD changed across the refused press");
    }
    const records = await fs
      .readdir(path.join(w.sandboxDir, ".archetipo", "executions"))
      .catch(() => []);
    if (records.length !== 0) {
      throw new Error(`AC-5: a refused press must create no execution record; found ${JSON.stringify(records)}`);
    }
    ok("AC-5", `the action is offered as not runnable stating ${JSON.stringify(truncate(inception.unavailable_reason))}, pressing it answers 409, the document is byte-identical and no record was written`);
  } finally {
    await w.close();
  }
}
...
// installs the inception skill the prompt invokes), starts the control server
// and the viewer, and saves the real claude provider pointed at the fake binary
// as the workspace default — which is what a person does in the Execution panel.
...
// writePRD persists the document through the CLI, exactly as the skill does.
...
// response is still in flight. Pushing the announcement before the press would
// name a session the viewer has not assigned yet; pushing it after would be a
// test waiting for the answer to a frame it is holding.
...
// announceSession queues the frame that opens the next turn's process.
//
// A streaming Claude exits when a turn ends, so every turn after the first is a
// new process resumed on the same session id, and every one of them announces
// itself with its own init. Queuing it before the command that starts the turn
// is what lets the fake answer a process that does not exist yet.
...
function printHelp() {
  console.log(`Smoke test for starting the inception from archetipo view against a fake agent binary

Usage:
  node ./test/e2e/inception-from-view-smoke.mjs
  npm run test:view-inception-smoke
...
<head>
  ...
  <title>ARchetipo Smoke — Inception from the viewer (US-040)</title>
  <style>
...
<body>
  <main>
    <header>
      <h1>ARchetipo Smoke — Inception from the viewer (US-040)</h1>
      <div class="meta">
    ...
    driven frame by frame: a workspace without a PRD that runs the inception as a conversation and gets its
    document, a workspace whose run is stopped halfway, and a workspace that already has a PRD.</p>
