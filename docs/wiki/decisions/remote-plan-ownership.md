...
La stessa proprietà vale per l'implementazione, che il provider dichiara accanto alla pianificazione. Cambiano la skill invocata e lo stato preteso dalla ricevuta — `REVIEW` invece di `PLANNED`, con il conteggio dei task svolti e una riga sull'esito dei test — mentre il resto della struttura è identico, e deve esserlo: un task spedito chiedendo una ricevuta e chiuso con l'altra lascerebbe un agente dichiarare un piano per chiudere un'implementazione. Il prompt di implementazione è quello che i provider locali già mandano, parola per parola oltre la frase di apertura, perché i tre accettano la stessa ricevuta e uno che chiedesse qualcosa di diverso ne produrrebbe di irricevibili per gli altri.

Ne seguono due prerequisiti operativi espliciti, e restano due perché hanno proprietari e cicli di vita diversi: la macchina ha installate la CLI e le skill ARchetipo, e la sua working directory è un checkout dello stesso progetto che condivide lo stato del connector configurato. Il primo cambia quando ARchetipo viene ricostruito, il secondo quando cambia il progetto su cui si lavora, e nessuno dei due appartiene all'immagine del runner — che resta una macchina ARcipelago generica.
...
Il prerequisito ha una forma esatta, ed è dichiarato invece che assunto. L'API esterna continua a non accettare `cwdHint`, `skills`, `assigneeAgentId` né `targetRunnerId`, e non è una lacuna: una credenziale applicativa è autorizzata sui workspace e su nient'altro, quindi un campo che le lasciasse nominare una directory — o una macchina — la porterebbe oltre il proprio ambito. Quei valori li dichiara il workspace, che è amministrato da chi possiede la flotta, e il task li eredita: `cwdHint`, `model` e `targetRunnerId` scendono da lì esattamente come già facevano sul percorso umano.

La regola che decide cosa un chiamante esterno può dire è una sola: **un campo è accettabile se e solo se può soltanto restringere**. `requirements` la soddisfa — è confrontato come sottoinsieme di ciò che un runner espone, quindi riduce l'insieme delle macchine ammissibili e non può allargarlo — ed è perciò l'unico campo aggiunto alla richiesta esterna, unito a quelli del workspace anziché sostituirli.

I due vincoli si dichiarano così: il workspace richiede una capability per ciascuno — per convenzione `archetipo` per il tooling e `project:<slug>` per il checkout — e il runner che li porta le espone. Sono due tag e non uno perché li scrive chi li rende veri: l'installazione del tooling scrive il proprio, accanto a `archetipo:<version>` che rende visibile una flotta ferma alla build di ieri, mentre il montaggio del checkout è deciso dove si definisce il container. Un tag unico costringerebbe l'uno a promettere anche per l'altro, che è esattamente ciò che rende una capability una dichiarazione anziché un fatto. Il dispatcher non assegna a un runner che non la porta — ma lo fa in silenzio, lasciando il task in coda a tempo indeterminato, che per un chiamante remoto è la risposta peggiore possibile. Perciò la creazione stessa rifiuta con `409 no_eligible_runner`, nominando la capability mancante, quando nessun runner noto all'hub potrebbe mai soddisfare la richiesta; un runner soltanto spento resta un'attesa e non un rifiuto, altrimenti l'hub respingerebbe lavoro a ogni riavvio di una macchina. La stessa domanda è leggibile prima di spedire qualsiasi cosa, da `GET /api/external/workspaces` e quindi da `archetipo doctor`.
