...
- `RunCollaborator` (`cli/internal/execution/run.go`) è la capacità opzionale di osservare e comandare una run, scoperta a runtime con `RunCollaboratorFor`. La implementano ARcipelago e, dopo US-038 e US-039, entrambi i provider locali: un nuovo provider **estende quell'interfaccia, non ne crea una parallela.** Le capacità sono già esposte alla UI da `cli/internal/web/execution.go`, e `run.dialog` è derivata dall'interfaccia realmente implementata — non va mai dichiarata a mano.
- Nessuno dei due provider locali gira più one-shot. La pianificazione con Codex apre una sessione viva su `codex app-server --listen stdio://` (JSON-RPC su stdio, codex-cli 0.147.0); quella con Claude apre una sessione viva su `claude --print --input-format stream-json --output-format stream-json --verbose --replay-user-messages` (NDJSON su stdio, Claude Code 2.1.235). In entrambi i casi è l'unica superficie della CLI che accetti un messaggio mentre l'agente lavora: il dialogo richiede una sessione viva, non un flag in più. `--replay-user-messages` non è opzionale — è ciò che fa ri-emettere al processo il messaggio dell'operatore, e la regola comune è che un messaggio entri nella storia solo quando il processo lo ri-emette. Le regole comuni del dialogo, e il vocabolario neutro degli eventi, vivono in `cli/internal/execution/localrun` e non vanno duplicate per provider.
