...
func (s *Server) sendNativeConversationMessage(ctx context.Context, ws *workspaceSession, snapshot conversationSnapshot, body sendConversationMessageReq) error {
	...
	if record.Work != "" && record.Work != execution.SessionIdle {
		...
		// A turn that ended under the message is not a failure of the message.
		// The end of a turn is a fact that arrives, not a fact that is known,
		// so whatever Work said an instant ago, the turn this steering named is
		// over — and the person's words still have to be delivered. Fall
		// through and open the next turn instead of losing them. Only that
		// refusal falls through: any other failure decided nothing and stays a
		// failure, with the delivery left uncertain for the reconciler.
		if reason, refused := execution.RefusalOf(steerErr); !refused || reason != execution.RunRefusedNotActive {
			return iox.NewConflict("the steering delivery was not confirmed", "reconcile the native session before retrying", steerErr)
		}
		delivery.State = execution.DeliveryUnsent
		record.Deliveries = upsertDelivery(record.Deliveries, delivery)
		record.Work = execution.SessionIdle
		if saveErr := ws.conversationStore().Save(context.WithoutCancel(ctx), record); saveErr != nil {
			return saveErr
