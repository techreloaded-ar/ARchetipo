...
type persistentFakeNativeProvider struct {
	...
	failStart bool
	// steerFindsNoTurn makes the fake answer a steering the way a runtime does
	// when the turn ended between the read and the command: the typed refusal,
	// and the session already idle.
	steerFindsNoTurn bool
...
func (p *persistentFakeNativeProvider) SteerTurn(_ context.Context, request execution.SessionCommandRequest) (execution.SessionDelivery, error) {
	if p.steerFindsNoTurn {
		p.store.mu.Lock()
		session := p.store.sessions[request.Session.Native.ID]
		completed := time.Now().UTC()
		if session.snapshot.CurrentTurn != nil {
			session.snapshot.CurrentTurn.State = execution.TurnCompleted
			session.snapshot.CurrentTurn.CompletedAt = &completed
		}
		session.snapshot.Work = execution.SessionIdle
		p.store.mu.Unlock()
		return execution.SessionDelivery{SubmissionID: request.SubmissionID, TurnID: request.TurnID, State: execution.DeliveryUncertain},
			&execution.RunCommandError{Reason: execution.RunRefusedNotActive, Err: fmt.Errorf("no active turn to steer")}
	}
...
// A message written in the instant a turn ends must not be lost. The end of a
// turn is a fact that arrives on another goroutine, so whatever the session
// said an instant ago, a steering can find the turn already over — and the
// person's words still have to be delivered, into the next turn. Only that
// refusal falls through: a fault decided nothing and stays a conflict, with
// the delivery left uncertain, which is what the sibling test asserts.
func TestSteeringRefusedByAFinishedTurnOpensTheNextOneInstead(t *testing.T) {
	provider := newPersistentFakeNativeProvider("native-fake", nil)
	srv, _, _ := newRunServer(t, provider, true)
	id := openConversationOK(t, srv).Conversation.ID

	if first := doJSON(t, srv, "POST", conversationsRoute+"/"+id+"/messages", map[string]any{"message": "opens the first turn"}); first.Code != 202 {
		t.Fatalf("first message = %d: %s", first.Code, first.Body.String())
	}
	if provider.store.starts != 1 {
		t.Fatalf("starts after the first message = %d, want 1", provider.store.starts)
	}

	provider.steerFindsNoTurn = true
	second := doJSON(t, srv, "POST", conversationsRoute+"/"+id+"/messages", map[string]any{"message": "written as the turn ended"})
	if second.Code != 202 {
		t.Fatalf("a message written as the turn ended = %d, want 202: %s", second.Code, second.Body.String())
	}
	if provider.store.starts != 2 {
		t.Fatalf("starts = %d, want 2: the refused steering did not open the next turn", provider.store.starts)
	}
	if provider.lastStart.Message != "written as the turn ended" {
		t.Fatalf("the next turn carries %q, want the message the steering could not deliver", provider.lastStart.Message)
	}
	...
	for _, delivery := range record.Deliveries {
		if delivery.State == execution.DeliveryUncertain {
			t.Fatalf("a delivery is still uncertain after the fallback and would block the next message: %#v", record.Deliveries)
		}
	}
}
